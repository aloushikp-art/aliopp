...
// Auto-size background word so it never overflows.
// Bold uppercase chars are ~0.58× font-size wide (with letter-spacing).
...
// ─── Dissolve variants ────────────────────────────────────────────────────────
// All product content (bg word + text + image) dissolves as a single unit.
const dissolveV = {
  enter: (d: number) => ({
    opacity: 0,
    x: d > 0 ? 28 : -28,
...
// Reduced-motion: plain crossfade only
...
function CupSVG({ size }: { size: number }) {
  return (
    <svg width={size} height={Math.round(size * 1.35)} viewBox="0 0 180 243" fill="none"
      ...
      <path d="M31 70 L38 222 Q39 237 56 237 L124 237 Q141 237 142 222 L149 70 Z" fill="rgba(255,255,255,0.06)" />
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
      {Array.from({ length: total }, (_, i) => (
        <button key={i} onClick={() => onDot(i)} aria-label={`Product ${i + 1}`}
          style={{
...
function ProductPanel({ drink, dir, reducedMotion }: {
  ...
  const cupSize = typeof window !== 'undefined' ? Math.min(200, Math.max(120, window.innerHeight * 0.28)) : 160
  ...
  return (
    ...
    >
      ...
      <div style={{ position: 'absolute', inset: 0, overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', pointerEvents: 'none', zIndex: 0 }}>
        <span style={{ fontSize: bgWordFontSize(drink.shortName), fontWeight: 900, letterSpacing: '-0.04em', lineHeight: 1, color: 'rgba(255,255,255,0.11)', whiteSpace: 'nowrap', userSelect: 'none' }}>
          {drink.shortName}
        </span>
      </div>

      {/* Product grid */}
      <div className="cd-hero-grid" style={{ position: 'relative', zIndex: 1, flex: 1, display: 'grid', gridTemplateColumns: 'minmax(0,1fr) minmax(0,1fr)', gap: 'clamp(12px,3vw,48px)', alignItems: 'center', maxWidth: 1180, width: '100%', margin: '0 auto', padding: 'clamp(8px,2vh,20px) clamp(56px,8vw,96px)' }}>
        {/* Text */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'clamp(10px,2vh,20px)' }}>
          <p style={{ margin: 0, fontSize: 'clamp(9px,1.1vw,12px)', fontWeight: 700, letterSpacing: '0.3em', color: 'rgba(255,255,255,0.52)', textTransform: 'uppercase' }}>Cold Drinks</p>
          ...
          <p style={{ margin: 0, fontSize: 'clamp(13px,1.3vw,17px)', color: 'rgba(255,255,255,0.65)', lineHeight: 1.6, maxWidth: 340 }}>{drink.description}</p>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          {drink.image ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
              <CupSVG size={cupSize} />
              <p style={{ margin: 0, fontSize: 10, letterSpacing: '0.16em', color: 'rgba(255,255,255,0.36)', textTransform: 'uppercase', fontWeight: 600 }}>Image Soon</p>
            </div>
          )}
        </div>
      </div>
...
function CarouselCard({ drink, dist, cardW, cardH, onClickCard }: {
  drink: typeof coldDrinks[number]; dist: number; cardW: number; cardH: number; onClickCard: () => void
}) {
  const abs = Math.abs(dist)
  return (
    <motion.div
      animate={{ scale: abs === 0 ? 1 : abs === 1 ? 0.88 : 0.77, opacity: abs === 0 ? 1 : abs === 1 ? 0.65 : abs === 2 ? 0.28 : 0 }}
      transition={{ duration: 0.42, ease: CINEMATIC }}
      onClick={onClickCard}
      style={{ width: cardW, height: cardH, flexShrink: 0, borderRadius: 22, background: `linear-gradient(155deg, ${drink.bgColor}f0, ${drink.bgColor}a0)`, border: '1px solid rgba(255,255,255,0.2)', cursor: abs === 0 ? 'pointer' : 'default', overflow: 'hidden', display: 'flex', flexDirection: 'column', boxShadow: abs === 0 ? '0 28px 72px rgba(0,0,0,0.38)' : '0 8px 24px rgba(0,0,0,0.18)', filter: abs > 1 ? 'blur(2px)' : 'none', transition: 'box-shadow 0.3s, filter 0.3s' }}
    >
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px 20px 12px' }}>
        {drink.image ? (
          <img src={drink.image} alt={drink.name} draggable={false} style={{ maxHeight: '100%', maxWidth: '100%', objectFit: 'contain', userSelect: 'none' }} />
        ) : (
...
function CarouselView({ initialIdx, onSelect, onBack }: {
  ...
  const onPD = (e: React.PointerEvent<HTMLDivElement>) => { dragging.current = true; dragStartX.current = e.clientX; trackStart.current = trackX.get(); (e.currentTarget as HTMLElement).setPointerCapture(e.pointerId) }
  ...
  useEffect(() => {
    const h = (e: KeyboardEvent) => { if (e.key === 'ArrowLeft') setIdx(i => Math.max(0, i - 1)); else if (e.key === 'ArrowRight') setIdx(i => Math.min(coldDrinks.length - 1, i + 1)) }
    window.addEventListener('keydown', h); return () => window.removeEventListener('keydown', h)
  }, [])

  return (
    <motion.div key="carousel" initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.97 }}
    ...
    >
      <motion.button initial={{ opacity: 0, y: -14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.18, duration: 0.42, ease: CINEMATIC }}
        onClick={onBack}
      ...
      <div ref={containerRef} style={{ flex: 1, width: '100%', overflow: 'hidden', cursor: 'grab', touchAction: 'pan-y', display: 'flex', alignItems: 'center' }}
      ...
      <div style={{ display: 'flex', gap: 12 }}>
        {(['left', 'right'] as const).map(d => (
          <button key={d} onClick={() => snap(d === 'left' ? idx - 1 : idx + 1)}
            ...
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ transform: d === 'right' ? 'scaleX(-1)' : 'none' }}><path d="M15 18l-6-6 6-6" /></svg>
          </button>
        ))}
      </div>
...
export default function ColdDrinksPage({ navigate }: { navigate: (to: NavRoute) => void }) {
  ...
  const onPD = useCallback((e: React.PointerEvent) => {
    if (view !== 'hero') return
    pointerStartX.current = e.clientX; pointerStartY.current = e.clientY; isDragHero.current = false
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
      ...
      {/* Center glow */}
      <div style={{ position: 'absolute', inset: 0, zIndex: 1, pointerEvents: 'none', background: 'radial-gradient(ellipse 55% 45% at 50% 50%, rgba(255,255,255,0.055) 0%, transparent 80%)' }} />
      ...
      <div style={{ flex: 1, position: 'relative', zIndex: 2 }}>
        <AnimatePresence mode="wait">
          {view === 'hero' ? (
            ...
            <CarouselView key="carousel" initialIdx={carouselInit}
